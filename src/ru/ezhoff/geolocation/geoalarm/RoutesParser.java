...
public class RoutesParser {
    ...
    private final static String ATTR_ROUTE_OFFICIAL_NAME    = "officialName";
    ...
    public List<Route> parse(XmlPullParser parser) throws XmlPullParserException, IOException {
        parser.next();
        parser.next();
        readRoutes(parser);
        return routes;
        ...
        while (parser.next() != XmlPullParser.END_DOCUMENT) {
            if (parser.getEventType() == XmlPullParser.START_TAG) {
                ...
                switch (parser.getName()) {
                    ...
                    case TAG_STATION_NAME       : currentTag = null; break;
            ...
            } else if (TAG_STATION_NAME.equals(currentTag) && parser.getEventType() == XmlPullParser.TEXT) {
                station.setName(parser.getText());
            }
        }
    }
    ...
    private double extractDouble(XmlPullParser parser, String name) {
        return parser.getAttributeValue(ns, name).matches("-?\\d+(\\.\\d+)?")
                ? Double.parseDouble(parser.getAttributeValue(ns, name))
                : 0d;
    }
